...
def update_bowling_stats(pl,last_n_stat):
    ...
    if last_n_stat.balls_bowled != 0:
            ...
            i,d = divmod(last_n_stat.overs_bowled,1)
            last_n_stat.economy = round(float(last_n_stat.runs_conceded)/(i + 100*d/6),2)
...
def calculate_batting_score(stat,agg_score,n):
    ...
    if stat.matches_played < n:
        ...
        score = round(stat.average * stat.strike_rate * sqrt(float(stat.batt_innings)/float(stat.matches_played)),2)
    return score
...
def calculate_bowling_score(stat,agg_score,n):
    ...
    if stat.matches_played < n:
        ...
        if stat.bowl_innings > 0 and stat.runs_conceded == 0:
            print '*********************************************'
            stat.runs_conceded = 1
            stat.economy = stat.runs_conceded/stat.overs_bowled
        if stat.economy * stat.strike_rate != 0:
            score = round((sqrt(float(stat.bowl_innings)/float(stat.matches_played)))/(stat.economy * stat.strike_rate),7)
    return score
...
def get_last_n_for_a_match(name,batt_agg,bowl_agg,n):
    ...
    #print players[players.keys()[0]][0].matches_played
    index = {3:'/home/shashank/intern_17/data/last_3_scores/',4:'/home/shashank/intern_17/data/last_4_scores/',5:'/home/shashank/intern_17/data/last_5_scores/',6:'/home/shashank/intern_17/data/last_6_scores/',7:'/home/shashank/intern_17/data/last_7_scores/',8:'/home/shashank/intern_17/data/last_8_scores/'}
    write_path = index[n]+ name
...
if __name__ == '__main__':
    ...
    season_9_files = [(str(i) + '.yaml') for i in range(980901,981020)]
    ...
    for n in range(7,9):
        agg_scores = get_last_n_agg_scores(n)
        for i in range(len(onlyfiles)):
            print n,i
            get_last_n_for_a_match(onlyfiles[i],agg_scores[0],agg_scores[1],n)
